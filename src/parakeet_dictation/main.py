...
class WhisperDictationApp(rumps.App):
    def __init__(self):
        ...
        self.is_toggle_recording = False
        self.toggle_key = os.getenv("PARAKEET_TOGGLE_KEY", "§")

        # Set up global hotkeys (Ctrl+Alt+A, toggle key) and release listener
    ...
    def monitor_keys(self):
        """
        Two recording modes:
        1. Hold Ctrl+Alt+A to record, release to stop (hold-to-record).
        2. Press § once to start recording, press § again to stop (toggle).
        """
        def start():
        ...
        def on_key_press(key):
            # Toggle recording with § key
            try:
                if hasattr(key, 'char') and key.char == self.toggle_key:
                    if not self.recording:
                        self.is_toggle_recording = True
                        logger.info(f"STARTING recording via {self.toggle_key} key")
                        self.start_recording()
                    elif self.is_toggle_recording:
                        logger.info(f"STOPPING recording via {self.toggle_key} key")
                        self.is_toggle_recording = False
                        self.stop_recording()
        ...
        logger.info(f"Starting global hotkey listener: Ctrl+Alt+A (hold) / {self.toggle_key} (toggle)")
        try:
